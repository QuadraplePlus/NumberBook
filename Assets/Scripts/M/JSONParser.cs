...
namespace JSONSLData
{
    public class JSONParser : MonoBehaviour
    {
        ...
        public void Save(Text name, Text number, Text email)
        {
            ...
            //기존연락처
            //PersonInfo p1 = new PersonInfo("홍길동", "1234-5678", "hong@naver.com");

            //추가 시켜줄 새 연락처
            PersonInfo pp = new PersonInfo(name.text, number.text, email.text);

            //personInfoList.personInfos[3].Name = name.text;
            //personInfoList.personInfos[3].Number = number.text;
            //personInfoList.personInfos[3].Email = email.text;


            //personInfoList.personInfos[i] = new PersonInfo(name.text, number.text, email.text);
